  ...
  <div class="flex flex-col gap-0.5">
    {#if $communities.length === 0}
      <div class="text-center p-3 pt-8">
        ...
        <p class="mt-1 text-sm text-gray-500">Get started by creating a new community or follow your favourite
          community.</p>
      </div>
    {/if}
    {#each $communities as community, i}
      <InertiaLink href="/@{community.prefix}" on:click={() => slideCommunity.set(false)} class="flex p-4 group">
        ...
        <div class="ml-3">
          <p
            class="text-sm font-medium text-gray-800 group-hover:text-gray-900 dark:text-gray-200 dark:group-hover:text-gray-50">{community.name}</p>
