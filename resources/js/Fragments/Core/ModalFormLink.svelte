...
<Modal bind:isOpen={$modalFormLink} maxW="max-w-md" forceClose={!$form.processing}>
    <div class="m-8">
        <div class="flex justify-between">
            <div class="text-center text-3xl font-extrabold text-1">Create Link</div>
        </div>
        ...
        <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
            <form class="space-y-6" on:submit|preventDefault={create}>
                ...
                <div>
                    ...
                    <div class="mt-1">
                        <textarea rows="4" id="link-note" class="{$form.errors.note ? 'input-error' : ''} block input" bind:value={$form.note}></textarea>
                    </div>
                    {#if $form.errors.note}
                ...
                <div>
                    ...
                    <div class="mt-1">
                        <InputTag bind:tags={$form.tags} bind:value={tagInputValue} bind:error={$form.errors.data} removeData={(i) => removeTag(i)} addData={(val) => addTag(val)}/>
                    </div>
                    {#if $form.errors.data}
                ...
                <div class="flex justify-end items-center gap-2">
                    ...
                    <button type="button" class="btn-simple btn-md" disabled={$form.processing} on:click={() => modalFormLink.set(false)}>Cancel</button>
                </div>
            </form>
        </div>
    </div>
